...
class OpenGLWidget(QOpenGLWidget):
    def __init__(self, parent=None):
        ...
        self.cube_z_rot = 30  # 30 degrees yaw for testing
    ...
    def draw_axes(self):
        ...
        # Apply shared Z-rotation
        glPushMatrix()
        glRotatef(self.cube_z_rot, 0.0, 0.0, 1.0)
    ...
    def draw_cube(self):
        ...
        # Apply cube's local rotation (around its center)
        glPushMatrix()
        glRotatef(self.cube_z_rot, 0, 0, 1)  # Apply yaw rotation
        vertices = (
    ...
    def reset_view(self, view_type):
        ...
        # Base view angles (before cube rotation adjustment)
        if view_type == 'right':
            x_rot_base = 0
            y_rot_base = 0     # Looking down -Y
        elif view_type == 'left':
            x_rot_base = 0
            y_rot_base = 180    # Looking down +Y
        elif view_type == 'top':
            x_rot_base = 90     # Looking down -Z
            y_rot_base = 0
        elif view_type == 'bottom':
            x_rot_base = -90    # Looking down +Z
            y_rot_base = 0
        ...
        # Adjust for cube's Z-rotation (yaw)
        # For XY-plane views (right/left/front/back), we modify y_rot
        # For Z-axis views (top/bottom), we modify x_rot
        if view_type in ['right', 'left', 'front', 'back']:
            self.y_rot = y_rot_base - self.cube_z_rot
            self.x_rot = x_rot_base
        elif view_type in ['top', 'bottom']:
            self.x_rot = x_rot_base
            self.y_rot = y_rot_base - self.cube_z_rot
        else:  # isometric and perspective
            self.x_rot = x_rot_base
            self.y_rot = y_rot_base - self.cube_z_rot * 0.7  # Partial adjustment for 3D views
